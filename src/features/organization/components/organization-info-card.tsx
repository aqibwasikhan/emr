...
export function OrganizationInfoCard({ org }: { org: Organization }) {
  ...
  return (
    <ExpandableCard
      editLinkHref={`/organization/edit/${org?.id}`}
      isExpanded={isExpanded}
      onToggle={() => setIsExpanded((prev) => !prev)}
    >
      <div className="flex transition-all duration-400 items-start gap-4 w-full">
        <div className="flex items-center justify-center bg-accent rounded-full size-12 text-lg font-medium text-primary transition-all duration-400">
          <OrgIcon className="h-6 w-6 text-secondary" />
        </div>
        <div className="flex-1 space-y-1">
          <CardTitle className="text-sm 2xl:text-base text-white transition-all duration-400">
            {org.name}
          </CardTitle>
          ...
          {isExpanded && (
            <div className="flex gap-4 flex-wrap text-white text-xs mt-4">
              {[
                { label: 'NPI', value: org.npiNumber },
                { label: 'Since', value: formatDate(org.createdAt) },
                { label: 'External Facility', value: org.facilityCount || '0' },
