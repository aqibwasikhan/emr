...
import { addRole, getModuleById, updateRole } from '@/app/actions/roles';
...
export default function SubRoleEditForm({
  config,
  initialValues,
  baseInitialValues
}: {
      ...
      formFields: section.formFields.map((field) => {
        if (field.name === 'module' && Array.isArray(field.options)) {
          const updatedOptions = field.options.map((option) => ({
            ...option,
            disable: !baseInitialModuleIds.includes(option.value),
          }));
  ...
  useEffect(() => {
    if (!initialized) {
      ...
      initialValues.modules?.forEach((mod: any) => {
    ...
    const fetchModules = async () => {
      try {
        const fetched = await Promise.all(
          newIds.map((id) => getModuleById(id, initialValues.id))
        );
    ...
    fetchModules();
  }, [selectedModules, initialized, initialValues.modules, modulesData]);
